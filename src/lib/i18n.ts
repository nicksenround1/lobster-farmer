export type Locale = "zh" | "en";

export const translations = {
  zh: {
    // Nav
    nav: {
      about: "关于",
      services: "服务",
      products: "产品",
      tools: "工具",
      community: "社区",
      contact: "联系",
    },
    // Hero
    hero: {
      title: "养虾户",
      subtitle: "Lobster Farmer",
      tagline: "养一池 AI 虾兵蟹将，替你 7×24 冲浪",
      description: "OpenClaw 资深玩家 · AI Agent 构建者 · Web3 投机专家",
      cta: "加入社区",
      learnMore: "了解更多",
    },
    // About
    about: {
      title: "关于我",
      p1: "我是一个 Web3 投机者、OpenClaw 深度用户、AI Agent 构建者。",
      p2: "我相信 AI Agent 是下一个生产力革命。通过 OpenClaw 平台，我构建了一套完整的 AI 自动化体系——从市场监控、Alpha 狙击到自动执行。",
      p3: "现在，我把这些能力开放给你。",
      highlight1: "6+",
      highlight1Label: "月 OpenClaw 深耕",
      highlight2: "20+",
      highlight2Label: "个 AI Agent 配置",
      highlight3: "24/7",
      highlight3Label: "全天候监控",
    },
    // Services
    services: {
      title: "服务",
      subtitle: "从零开始，帮你搭建 AI Agent 帝国",
      openclaw: {
        title: "养虾户 Bundle（指南 + Persona）",
        desc: "40+ 页实战指南 + 完整 AI Agent 配置包。从零搭建到 Web3 实战，一步到位。省 $9。",
        tag: "超值",
      },
      persona: {
        title: "养虾户 Persona 包",
        desc: "完整 AI Agent 配置：SOUL.md + 5 个 Skills + 配置指南。Crypto/Web3 专属，中文优化。即买即用。",
        tag: "热卖",
      },
      api: {
        title: "AI 模型 API 中转",
        desc: "Claude / GPT / Grok 等顶级模型的 API 中转服务。官方 API 直连，高速稳定，按量计费，支持 400+ 模型。",
        tag: "已上线",
      },
      alpha: {
        title: "Alpha 情报服务",
        desc: "Crypto KOL 监控、链上数据分析、叙事追踪。信息差就是利润差。",
        tag: "Alpha",
      },
      comingSoon: "即将上线",
      visitNow: "立即访问",
      buyNow: "立即购买",
      joinWaitlist: "加入候补",
      viewAll: "查看全部产品",
      first100: "前 100 位特价",
    },
    // Products
    products: {
      title: "产品",
      subtitle: "选择适合你的 AI Agent 配置包",
      filterAll: "全部",
      filterStarter: "入门",
      filterCore: "核心",
      filterPremium: "高端",
      filterBundle: "套餐",
      viewDetails: "查看详情",
      buyNow: "立即购买",
      comingSoon: "即将上线",
      contactBuy: "加群咨询",
      features: "功能特性",
      includes: "包含内容",
      relatedProducts: "相关产品",
      backToProducts: "← 返回产品列表",
      save: "省",
    },
    // Tools
    tools: {
      title: "技术栈",
      subtitle: "我的工具箱",
    },
    // Community
    community: {
      title: "加入社区",
      desc: "加入我们的 Telegram 群组，和一群 AI Agent 爱好者交流心得、分享 Alpha、共同成长。",
      join: "加入 Telegram 群",
      members: "成长中的社区",
    },
    // Contact
    contact: {
      title: "联系我",
      subtitle: "有想法？来聊聊。",
      twitter: "Twitter / X",
      telegram: "Telegram 群",
      email: "邮箱",
    },
    // Footer
    footer: {
      rights: "养虾户 · Lobster Farmer",
      tagline: "用 AI 养虾，用虾养 Alpha",
    },
  },
  en: {
    nav: {
      about: "About",
      services: "Services",
      products: "Products",
      tools: "Tools",
      community: "Community",
      contact: "Contact",
    },
    hero: {
      title: "养虾户",
      subtitle: "Lobster Farmer",
      tagline: "Raising a pond of AI lobsters to surf 24/7 for you",
      description: "OpenClaw Power User · AI Agent Builder · Web3 Speculator",
      cta: "Join Community",
      learnMore: "Learn More",
    },
    about: {
      title: "About Me",
      p1: "I'm a Web3 speculator, OpenClaw power user, and AI agent builder.",
      p2: "I believe AI agents are the next productivity revolution. Through OpenClaw, I've built a complete AI automation system — from market monitoring and alpha sniping to automated execution.",
      p3: "Now, I'm making these capabilities available to you.",
      highlight1: "6+",
      highlight1Label: "Months on OpenClaw",
      highlight2: "20+",
      highlight2Label: "AI Agent Configs",
      highlight3: "24/7",
      highlight3Label: "Always-On Monitoring",
    },
    services: {
      title: "Services",
      subtitle: "From zero to hero — build your AI Agent empire",
      openclaw: {
        title: "Lobster Farmer Bundle (Guide + Persona)",
        desc: "40+ page hands-on guide + full AI Agent config package. Zero to Web3 agent, all-in-one. Save $9.",
        tag: "Best Value",
      },
      persona: {
        title: "Lobster Farmer Persona Pack",
        desc: "Complete AI Agent config: SOUL.md + 5 Skills + setup guide. Crypto/Web3 focused, Chinese-optimized. Plug & play.",
        tag: "Popular",
      },
      api: {
        title: "AI Model API Gateway",
        desc: "API relay for top models — Claude, GPT, Grok & 400+ more. Direct official API access, high-speed, stable, pay-per-use.",
        tag: "Live",
      },
      alpha: {
        title: "Alpha Intelligence Reports",
        desc: "Crypto KOL monitoring, on-chain analytics, narrative tracking. Information asymmetry = profit.",
        tag: "Alpha",
      },
      comingSoon: "Coming Soon",
      visitNow: "Visit Now",
      buyNow: "Buy Now",
      joinWaitlist: "Join Waitlist",
      viewAll: "View All Products",
      first100: "First 100 Special",
    },
    products: {
      title: "Products",
      subtitle: "Choose the AI Agent config pack that fits you",
      filterAll: "All",
      filterStarter: "Starter",
      filterCore: "Core",
      filterPremium: "Premium",
      filterBundle: "Bundle",
      viewDetails: "View Details",
      buyNow: "Buy Now",
      comingSoon: "Coming Soon",
      contactBuy: "Join Group to Buy",
      features: "Features",
      includes: "What's Included",
      relatedProducts: "Related Products",
      backToProducts: "← Back to Products",
      save: "Save",
    },
    tools: {
      title: "Tech Stack",
      subtitle: "My Toolbox",
    },
    community: {
      title: "Join the Community",
      desc: "Join our Telegram group to exchange ideas, share alpha, and grow together with fellow AI agent enthusiasts.",
      join: "Join Telegram Group",
      members: "Growing Community",
    },
    contact: {
      title: "Get in Touch",
      subtitle: "Got ideas? Let's talk.",
      twitter: "Twitter / X",
      telegram: "Telegram Group",
      email: "Email",
    },
    footer: {
      rights: "养虾户 · Lobster Farmer",
      tagline: "Raising AI lobsters, farming alpha",
    },
  },
} as const;

// Deep mapped type to get string values instead of literal types
type DeepStringify<T> = {
  [K in keyof T]: T[K] extends string ? string : DeepStringify<T[K]>;
};

export type Translations = DeepStringify<typeof translations.zh>;
